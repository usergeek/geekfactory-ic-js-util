import {intlFormatNumber} from "geekfactory-js-util";
 ...
 */
export const formatNumberToMaximumDigits = (value: number, options?: FormatNumberToMaximumDigitsOptions): string => {
    ...
    if (wholePartLength >= maxDigits) {
        return intlFormatNumber(value, {maximumFractionDigits: 0, thousandSeparator: thousandSeparator})
    }
    const decimalPartDigits = maxDigits - wholePartLength
    return intlFormatNumber(value, {maximumFractionDigits: Math.max(0, decimalPartDigits), thousandSeparator: thousandSeparator})
}